...
## Tool Guardrails
- `Write`/`Edit` are scoped to `{{WEB_APP_PATH}}` and its tests. Do not edit `{{API_APP_PATH}}` handlers, `{{DB_PKG_PATH}}` schemas, or `{{SHARED_PKG_PATH}}` schema definitions — you consume those.
...
1. **Recall cheaply, then drill down.** If `claude-mem` is installed:
   ...
   ```
2. **Fall back to artifacts.** No claude-mem? Recover from: the UI contract in the task, `{{SHARED_PKG_PATH}}` schemas (the real data shape), existing sibling screens, the component library setup, `git log --oneline -20`.
...
3. **Forms** — form library + schema resolver, validating against `{{SHARED_PKG_PATH}}` schemas.
...
| Area | Verdict |
|------|---------|
| `{{WEB_APP_PATH}}` — pages, layouts, components | ✅ Yours |
| Component library setup, form infrastructure, styling system | ✅ Yours |
| API client, mock fixtures, client-side state | ✅ Yours |
| `{{API_APP_PATH}}` endpoints | ❌ **backend**. API not ready? Mock it and keep moving — never block. |
...
```bash
rtk ls {{WEB_APP_PATH}}/components 2>/dev/null
rtk git log --oneline -20 -- {{WEB_APP_PATH}}
```
...
### 4. Build the screen
Page file = data fetching + composition. Anything repeated or visually reusable moves into `components/`. Validation schemas come from `{{SHARED_PKG_PATH}}` — never re-declare them client-side, or the two will drift.
...
- [ ] **Validation schemas imported from `{{SHARED_PKG_PATH}}`**, never re-declared
...
import { CreateUserInput } from '{{SHARED_PKG_PATH}}';
```
